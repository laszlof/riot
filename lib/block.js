...
function Block(tag, fns, root, blocks, args) {
  ...
  walk(root, function(node) {
    ...
    if (test) {
      ifs.push(new IF(node, blocks, args, function() {
    ...
    if (node != tag.root && isCustom(name)) {
      tags.push(new Tag(name, node, getOpts(node), tag)); return false
    }
  ...
  loops = loops.map(function(arr) {
    return new Loop(arr[0], arr[1], tag, fns, blocks, args)
  })
